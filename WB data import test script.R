...
summary(WBdata$DBP)
dfdest <- Hypertdf[Hypertdf[,"SBP"]>150 & Hypertdf[,"SBP"]<220 & !is.na(Hypertdf[,"SBP"]),]
dfdest <- Hypertdf[Hypertdf[,"SBP"]>150 & Hypertdf[,"SBP"]<220 | is.na(Hypertdf[,"SBP"]),]
dfdest <-DataClean(Hypertdf,"SBP",150,220)
dfdest <-DataClean(WBdata,"DBP",0,120,naremove = FALSE)

PlotAsp(WBdata[WBdata$Hypert=="Yes",],"Agebands5","GenderMF",mtitle="Hypertension population")
CleanSBP <-DataClean(WBdata,"SBP",100,205)
CleanSBP <-DataClean(WBdata,"SBP",100,205,naremove = F)
